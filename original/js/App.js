define([ "ManageConnectors", "AddConnectors", "ManageVariables", "libs/jquery.form", "libs/jquery.jeditable.mini" ], function(
		ManageConnectors, AddConnectors,ManageVariables) {
	...
	function loadAppTemplates() {
		var apps = _.values(App.apps), appTemplates = apps.map(function(app) {
	...
	function setupURLRouting() {
		FlxState.router.route("*path", "default", function(path) {
			...
			var appName = FlxState.defaultApp, app = App.apps[appName];
			renderDefault(app);
		});
		FlxState.router.route("analyze/:name", "app-default", function(appName) {
			console.log("app-default route: name=" + appName);
			var app = App.apps[appName];
			renderDefault(app);
		});
		FlxState.router.route("analyze/:name/*state", "app", function(appName, state) {
	...
	function createAppsMenu() {
		$.each(FlxState.apps, function(i, appName) {
			var app = App.apps[appName], button = $("<button/>", {
	...
	App.fetchCompiledMustacheTemplate = function(templatePath, templateId) {
		...
		console.log("App.fetchCompiledMustacheTemplate(): Unknown template path [" + templatePath
				+ "] and/or template ID [" + templateId + "].  Returning null.");
		return null;
	};
	...
	function glow(element) {
		element.css("text-shadow", "0 0 10px white").css("color", "white");
	}
	...
	App.removeConnector = function(api) {
		var c = confirm("If you wrote comments on events related to this connector, " + "you will lose them forever.\n"
	...
	// if (config == null){
	// console.log("WARNING: No config found for Connector: " +
	// App.getFacetConnector(facetName));
	// config = ConnectorConfig.default;
	// }
	// var finalConfig = $.extend({},config);
	// finalConfig.facets = null;
	// if (config.facets == null ||
	// config.facets[App.getFacetObjectType(facetName)] == null){
	...
	App.addHideTooltipListener = function(hideFunction) {
		var onEvent = function(event) { // hides the tooltip if an element
										// clicked on or any of its parents has
										// the notthide property
	...
	function feedback() {
		App.loadMustacheTemplate("serviceTemplates.html", "feedbackBox", function(template) {
			var html = template.render();
			App.makeModal(html);
			$('#feedback-send').click(function() {
				$.ajax("/analyze/feedback/send", {
					type : "POST",
					data : {
	...
	function upload() {
		App.loadMustacheTemplate("serviceTemplates.html", "uploadBox", function(template) {
			var html = template.render();
			App.makeModal(html);
			$('#upload-send').click(function() {
				var files = new FormData();
				var filesElement = $('#files');
				$.each(filesElement[0].files, function(i, file) {
					files.append(filesElement.attr('name'), file);
				});
				$.ajax("/upload", {
					type: "POST",
					contentType: false,
