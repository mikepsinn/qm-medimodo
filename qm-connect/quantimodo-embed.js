(function connectjs() {
    ...
    function getApiUrl () {
        ...
        if (!apiUrl) {
            apiUrl = getClientId() + '.quantimo.do'
        }
    ...
    function getAppSettings (successHandler) {
        function getAppSettingsUrl () {
            return 'https://' + getApiUrl() + '/api/v1/appSettings?clientId=' + getClientId()
        }
    ...
    function getAPIQueryString() {
        var queryString;
        var moreInfo = '  See https://app.quantimo.do/api/v2/apps for more information.';
        ...
        if(window.QuantiModoIntegration.options.publicToken && window.QuantiModoIntegration.options.publicToken !== ''){
            queryString += '&publicToken=' + window.QuantiModoIntegration.options.publicToken;
        } else {
            console.warn('No window.QuantiModoIntegration.options.publicToken provided so assuming this a new user. ' + moreInfo);
        }
    ...
    function quantimodoApiCall(method, requestPath) {
        ...
        request.onload = function () {
            ...
            if(data && data.sessionTokenObject) {
                ...
                if(response.error && response.error.message){
                    ...
                    logError(JSON.stringify(response), response);
                }
            }
        };
    ...
    function postJsonToAPI(jsonObject, url) {
        var xhr = new XMLHttpRequest();
    ...
    function getOrCreateUser() {quantimodoApiCall("GET", '/api/v3/user');}
    function showPopup() {
    ...
    function renderConnectorList() {
        ...
        var connectorBlock = {
            ...
            '               <img class="qm-sync-image" src="https://app.quantimo.do/api/v2/img/sync-btn-red.png">' +
            '           <% } else if (!qmClient && connected) { %>' +
            '               <img class="qm-sync-image" src="https://app.quantimo.do/api/v2/img/sync-btn.png">' +
            '           <% } %>' +
    ...
    window.QuantiModoIntegration.qmSetupIonicPopupIframe = function (state) {
        ...
        window.onload = function() {
            ...
            qmMain.innerHTML = '<iframe style="height:100%;width:100%;" id="ionic-app-frame" src="https://app.quantimo.do/ionic/Modo/www/index.html#' + statePath + '" frameborder="0"></iframe>';
            showPopup();
            showLoader(false);
        };
    };
    ...
    window.QuantiModoIntegration.createSingleFloatingActionButton = function() {
        getAppSettings(function() {
            ...
            function getIframeQueryString() {
                return '?hideMenu=' + window.QuantiModoIntegration.options.hideMenu + '&apiUrl=' + getApiUrl();
            }
            ...
            qmPageElements.qmIonicAppSidebar.template =
                ...
                'src="https://' + getApiUrl() + '/ionic/Modo/www/index.html#/app/' + window.QuantiModoIntegration.options.defaultState + getIframeQueryString() + '">' +
                '</iframe>' +
                '</div>';
